...
export const AdminOrdersPage = () => {
  ...
  const handleDownloadPackingSlip = async () => {
    ...
    try {
      await orderService.downloadPackingSlip(selectedOrder.id, selectedOrder.order_number);
    } catch (error) {
      toast.error(getApiErrorMessage(error, 'Failed to download packing slip'));
    }
  };
  ...
  const handleDownloadInvoice = async () => {
    if (!selectedOrder) return;
    try {
      await orderService.downloadInvoice(selectedOrder.id, selectedOrder.order_number);
    } catch (error) {
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedOrder && (
            <div className="space-y-6 py-4">
              ...
              {/* Actions */}
              <div className="flex gap-2">
                <Button variant="outline" className="flex-1" onClick={handleDownloadPackingSlip}>
